...
namespace Feeling.GIS.Map.Core
{
   ...
   public class MercatorProjection : PureProjection
   {
      ...
      MapSize tileSize = new MapSize(256, 256);
      public override MapSize TileSize
      {
         get
         {
            return tileSize;
      ...
      public override MapSize GetTileMatrixMinXY(int zoom)
      {
         return new MapSize(0, 0);
      }

      public override MapSize GetTileMatrixMaxXY(int zoom)
      {
         int xy = (1 << zoom);
         return new MapSize(xy - 1, xy - 1);
      }
   }
}
